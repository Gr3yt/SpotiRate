...
async function searchAlbum() {
  ...
  try {
    const res = await fetch(`https://api.spotify.com/v1/search?q=${encodeURIComponent(query)}&type=album&limit=6`, {
...
function generateCard() {
  ...
  const trackRows = tracks.map((t, i) => {
    const r = ratings[i];
    const barW = (r / 10) * 100;
    const color = r <= 3 ? '#e05c5c' : r <= 6 ? '#e0b95c' : '#1DB954';
    return `
  ...
  document.getElementById('export-card').innerHTML = `
    ...
    <div class="card-footer">
      <span class="card-footer-brand">ALBUM RATER</span>
...
async function downloadCard(e) {
  ...
  try {
    ...
    link.download = `${currentAlbum?.name || 'album'}-rating.png`;
